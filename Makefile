...
# Get the version from digit/build.gradle, assuming a line like: version = 'x.y.z'
VERSION := $(shell grep -m 1 "version = '" digit/build.gradle | sed -e "s/.*version = '\(.*\)'.*/\1/")
TAG := v$(VERSION)
...
publish:
	...
	@echo "Version from digit/build.gradle: $(VERSION)"
